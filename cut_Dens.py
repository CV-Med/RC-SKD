...
def prune_conv1(conv_layer, prune_ratio):
    ...
    num_pruned_channels = int(conv_layer.out_channels * prune_ratio)  # 根据比例计算裁剪通道数
    # print(num_pruned_channels, conv_layer.out_channels)
    norms = weight.view(weight.size(0), -1).norm(2, dim=1)  # 计算每个通道的L2范数
    _, indices = torch.topk(norms, k=num_pruned_channels, largest=True)  # 获取最不重要的通道
...
def prune_conv2(conv_layer, remaining_indices):
    weight = conv_layer.weight.data
    pruned_weight = weight[:, remaining_indices, :, :]  # 只保留通道上残余的部分
...
def prune_bottleneck_block(layer, prune_ratio):
    ...
    if hasattr(layer, 'conv1') and hasattr(layer, 'conv2'):
        # 更新norm1层，先更新norm1再裁剪conv1
        indices = torch.arange(layer.conv1.in_channels)  # 使用conv1的输入通道数
        layer.norm1 = prune_bn_layer(layer.norm1, indices)  # 更新norm1

        # 裁剪conv1层
        new_conv1, indices = prune_conv1(layer.conv1, prune_ratio)
        layer.conv1 = new_conv1

        # 更新norm2层，更新时需要使用conv1的输出通道数
...
def train_eval(compressed_model, epochs, train_loader, val_loader):
    ...
    for epoch in range(epochs):
        ...
        if best_acc <= val_acc:
            ...
            model_name = f"./Weight/CI_D201_{best_acc:.3f}_model.pth"
...
def target_model(state):
    print(state)
    # 保存到 txt 文件
    output_file = "D201_state.txt"
    with open(output_file, "a") as f:
        f.write(f"{state}\n")  # 每个值占一行

    tea_model = models.densenet201(weights=DenseNet201_Weights.DEFAULT)
    # 修改最后的全连接层以适应新数据集
    ...
    reward = train_eval(stu_model, 150, train_loader, val_loader)
    # 保存到 txt 文件
    output_file = "D201_val.txt"
    with open(output_file, "a") as f:
        f.write(f"{reward}\n")  # 每个值占一行
...
if __name__ == '__main__':
    ...
    # 训练环境测试
    print(target_model([0.3, 0.478, 0.47, 0.3]))